...
<template>
  <div grid="~ cols-[auto_1fr_1fr] items-center gap-16" f-my-md class="nq-raw" max-w-800 relative>
    ...
    <div v-for="gradient in gradients" :key="gradient" contents>
      ...
      <button
        bg-transparent :style="getBg(`${gradient}`)" max-h-48 aspect-square w-full rounded="4 md:6"
        outline="~ 1 offset--1 black/10 dark:white/10" @click="setActiveGradient(gradient)"
      />
      <button
        bg-transparent :style="getBg(gradient, true)" max-h-48 aspect-square w-full rounded="4 md:6"
  ...
  <ClientOnly>
    <Teleport defer to="#widget">
      <div v-if="activeGradient">
        <div id="color-card" ref="card" size-48 w-full rounded-6 :style="cssVar" outline="1 ~ offset--1 black/10 dark:white/10" />
