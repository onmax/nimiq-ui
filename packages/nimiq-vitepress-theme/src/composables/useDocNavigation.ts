    ...
    for (const section of currentDocModule.value.sidebar) {
      for (const item of section.items) {
        if (item.link && (withBase(item.link) === route.path || `${withBase(item.link)}/` === route.path))
          return { item, section }
        if (item.items) {
          const nestedItem = item.items.find(subitem =>
