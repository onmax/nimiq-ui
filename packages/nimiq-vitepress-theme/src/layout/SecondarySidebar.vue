...
<template>
  <div f-text-xs sticky f="$h $h-min-60 $h-max-88" h="[calc(100vh-var(--f-h))]" f-top-xl f-px-sm w="[calc(var(--nq-sidebar-width)+24px)]" of-y-auto f-pb-xs>
      ...
      <li v-for="h2 in headingTree" :key="h2.hashPath" flex="~ col" pb-4>
        ...
        <ol v-if="h2.items.length">
          <li v-for="h3 in h2.items" :key="h3.hashPath" flex="~ col">
            <a :href="`#${h3.hashPath}`" font-normal p-4 pl-14 :class="{ 'text-blue font-semibold': isHeadingActive(h3.hashPath) }">
              {{ h3.text }}
            </a>
          </li>
        </ol>
    ...
    <div v-if="showWidget" id="widget" max-w-full :class="{ 'f-mt-md': showOutline }" h-max />
  </div>
</template>
