...
function generateCssVariables(colorData: typeof colors, gradients: Record<string, GradientColor>): string {
  ...
  for (const gradientName in gradients) {
    ...
    cssString += `  --colors-${kebabGradientName}-gradient: radial-gradient(at 100% 100% in oklab, var(--colors-${kebabGradientName}-gradient-from), var(--colors-${kebabGradientName}-gradient-to));\n`
    ...
    if (gradientDef.darkened
      ...
      cssString += `  --colors-${kebabGradientName}-gradient-darkened: radial-gradient(at 100% 100% in oklab, var(--colors-${kebabGradientName}-gradient-darkened-from), var(--colors-${kebabGradientName}-gradient-darkened-to));\n`
    }
  }
